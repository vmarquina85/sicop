...
mov_tar,mov_ofitra,mov_ofiper,mov_status) values('" . $fecha . "','5','" . $generado . "','" . rtrim($_SESSION['sicop_usr_id']) . "',now(),'2','" . rtrim($_GET['recibe']) . "','" . rtrim($_SESSION['usr_idper']) . "','" . rtrim($_SESSION['id_area']) . "','" . rtrim($_SESSION['id_dep']) . "','" . rtrim($_GET['area_d']) . "','" . rtrim($_SESSION['id_cargo']) . "','" . rtrim($_GET['cargo']) . "','" . rtrim($_GET['destino']) . "','" . rtrim($_GET['oficina_d']) . "','" . rtrim( $_SESSION['id_oficina']) . "','I')";
pg_query($cn, $cad1);
//Graba detalle
$ni = count($matriz);
for ($i = 0;$i < $ni;$i++) {
